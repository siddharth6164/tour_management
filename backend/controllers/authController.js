...
export const register = async(req,res)=>{
    try {
        ...
        res.status(500).json({success: false,message:"Failed to create. Try Create"})       
    }
}
//user login
export const login = async(req,res)=>{
    ...
    try {
        ...
        //create jwt token
        const token = jwt.sign({id:user._id,role:user.role},process.env.JET_SECRET_KEY,{expiresIn:"15d"})
